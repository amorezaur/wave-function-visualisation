...
const Example = () => {
	...
	const getAxisYDomain = (
		...
		const refData = initialData.slice(Number(from) - 1, Number(to));
	...
	const zoom = () => {
		...
		const { dataSource } = waveGraphState;
		...
		console.log("refAreaLeft ,refAreaRight", refAreaLeft, refAreaRight);
	...
	const zoomOut = () => {
		const { dataSource: data } = waveGraphState;
		setWaveGraphState((data) => ({
			...data,
			dataSource: dataSource.slice(),
			refAreaLeft: "",
